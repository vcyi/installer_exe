...
首次启动会编译制作台源码，随后直接打开 Windows 桌面窗口。制作台默认读取同目录的 `build-config.json`，可导入、导出或保存配置。

| 配置分类 | 可设置内容 |
|---|---|
| 产品与目录 | 产品名称、版本、发布者、副标题、基础程序目录、主 EXE、默认安装目录、输出目录、图标 |
| 安装模式 | 是否允许最终用户选择自定义安装；是否允许基础/快速安装选择路径（`allowInstallPathSelection`） |
| 系统 Path | `addToSystemPath`：将安装目录，或已解析 `{app}` 的环境变量值加入 HKLM 系统 Path（需要管理员权限） |
| 快捷方式 | 是否创建桌面快捷方式、开始菜单快捷方式 |
| 启动项 | 是否写入当前用户启动项、启动项名称、启动参数 |
| 环境变量 | 是否写入当前用户环境变量、变量名称、变量值；`{app}` 会替换为实际安装目录 |
| 外部资源 | 资源名称、下载 URL、是否必选、解压路径、哈希值 |
| 卸载清理 | 是否删除桌面快捷方式、开始菜单、启动项、环境变量和整个安装目录 |
...
- 删除当前用户启动项。
- 删除由本安装包设置的当前用户环境变量。
- 仅删除本安装包实际写入的那一项系统 Path；保留其余 Path 内容并发送环境变量变更通知。
- 删除卸载注册表项。
