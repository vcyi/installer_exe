...
public class InstallerStudioNative : Form
{
    ...
    CheckBox customInstall, allowInstallPathSelection, addToSystemPath, desktop, startMenu, startup, writeEnv, cleanDesktop, cleanStartMenu, cleanStartup, cleanEnv, cleanInstallDir;
    ComboBox theme;
    ...
    void BuildBehaviorPage()
    {
        ...
        AddRow(bp, 0, "安装模式", customInstall); AddRow(bp, 1, "基础安装路径", allowInstallPathSelection); AddRow(bp, 2, "系统 Path", addToSystemPath); AddRow(bp, 3, "桌面快捷方式", desktop); AddRow(bp, 4, "开始菜单", startMenu); AddRow(bp, 5, "启动项", startup); AddRow(bp, 6, "启动项名称", startupName); AddRow(bp, 7, "启动参数", startupArgs); AddRow(bp, 8, "环境变量", writeEnv); AddRow(bp, 9, "变量名称", envName); AddRow(bp, 10, "变量值", envValue); AddRow(bp, 11, "卸载清理", cleanDesktop); AddRow(bp, 12, "", cleanStartMenu); AddRow(bp, 13, "", cleanStartup); AddRow(bp, 14, "", cleanEnv); AddRow(bp, 15, "", cleanInstallDir); AddRow(bp, 16, "安装界面主题", theme);
    }
